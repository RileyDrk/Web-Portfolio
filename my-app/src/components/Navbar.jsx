...
function ThemeToggleInner({ darkMode, onToggle, compact }) {
  ...
  return (
    <button
      type="button"
      onClick={onToggle}
      className={`relative shrink-0 ${track} rounded-full border border-theme bg-surface-muted shadow-inner transition-[background-color,box-shadow] duration-[var(--duration-theme)] ring-2 ring-transparent hover:ring-[var(--accent-soft)]`}
    ...
    >
      ...
      <span
        className={`pointer-events-none absolute top-1/2 z-[2] ${knob} -translate-y-1/2 rounded-full border border-theme bg-[var(--surface)] shadow-md transition-[left] duration-[450ms] ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:duration-150 motion-reduce:transition-none`}
        style={{
...
function SocialIcons({ compact = false }) {
  const base =
    "inline-flex items-center justify-center rounded-xl border border-theme bg-surface-muted/80 text-muted transition-[color,background-color,transform,border-color] duration-200 hover:bg-[var(--accent-soft)] hover:border-[color-mix(in_oklch,var(--accent)_30%,transparent)] hover:text-[var(--accent)] active:scale-[0.96]"
...
export default function Navbar() {
  ...
  return (
    <nav className="glass-nav border-b border-theme sticky top-0 z-50 shadow-[var(--shadow-card)] transition-shadow duration-[var(--duration-theme)]">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex items-center justify-between h-[4.125rem]">
          ...
          <div className="flex items-center gap-2 md:hidden">
            ...
            <button
              type="button"
              className="p-2.5 rounded-xl border border-theme bg-surface-muted/90 text-theme transition-[transform,background-color] duration-300 hover:bg-surface-muted active:scale-[0.97]"
          ...
          <div className="md:hidden mobile-menu-sheet border-t border-theme py-5 flex flex-col gap-1">
            {links.map(({ href, label }) => (
              <a
                key={href}
                href={href}
                className="py-3.5 px-3 rounded-xl text-theme font-semibold transition-[background-color,transform,color] duration-300 hover:bg-[var(--accent-soft)] active:translate-x-0.5"
